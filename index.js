...
router.get('/x', async re =>  {
  TOKEN = re.query.t
                ...
                // }
                var i = "-" + Date.now()
                re.photo = `https://res.cloudinary.com/o6/image/upload/c_scale,w_1280/b_aquamarine,co_black,l_text:Yanone%20Kaffeesatz_42_bold_center:%20${re.query.ll.replace(/,/g, "%20") + "%20" + re.query.ref}%20${re.query.cap.replace(/ /g, "%20")}%20,fl_relative,w_1,y_1.01,g_south/l_i:${re.file},fl_relative,w_1,y_1.01,g_south/${re.query.geo}`
 
          re.photo = await fetch(`https://api.cloudinary.com/v1_1/o6/image/upload?public_id=${re.file}&upload_preset=o6oooo&file=${encodeURIComponent(re.photo)}`).then(r => r.json()).then(r => {
